			...
			requestMap.entrySet().forEach(parameter -> {
				...
				if (isFieldName) {
					...
					switch (firstField.getType().getTypeName()) {
						...
						case "review.exercise.Gender": {							
							// Set the typeInstance field to the parameter value
							try {
								Gender gender = Gender.valueOf(stringValue);
								try {
									Method setValueMethod = classType.getDeclaredMethod(methodName, Gender.class);
									setValueMethod.invoke(typeInstance, gender);
								} catch (Exception ex) {
									JsonObject exceptionJsonObject = Json.createObjectBuilder()
										.add(parameter.getKey(), ex.getMessage())
										.build();
