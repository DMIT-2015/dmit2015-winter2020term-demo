...
public class EPSNeighbourhoodCriminalOccurrencesManager {
	...
	public EPSNeighbourhoodCriminalOccurrencesManager() throws IOException {
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(getClass().getResourceAsStream("/data-edmonton-ca/EPS_Neighbourhood_Criminal_Occurrences.csv"))))	{			
			String line;
	...
	public List<EPSNeighbourhoodCriminalOccurrences> findOccurrencesByNeighbourhoodDescription(String neighbourhoodDescription) {
		return criminalOccurences.stream()
				.filter(item -> item.getNeighbourhoodDescription().equalsIgnoreCase(neighbourhoodDescription))
				.sorted(Comparator.comparingInt(EPSNeighbourhoodCriminalOccurrences::getOccurrenceReportedYear).reversed()
						.thenComparing(
							Comparator.comparing(EPSNeighbourhoodCriminalOccurrences::getOccurrenceReportedQuarter))
						.thenComparing(
							Comparator.comparingInt(EPSNeighbourhoodCriminalOccurrences::getOccurrenceReportedMonth))
						)
				.collect(Collectors.toList());
	}
	
	public List<EPSNeighbourhoodCriminalOccurrences> findOccurrencesByNeighbourhoodDescriptionAndOccurrenceReportedYear_OccurrenceReportedQuarter(
			String neighbourhoodDescription, int occurrenceReportedYear) {
		return criminalOccurences.stream()
				.filter(item -> item.getNeighbourhoodDescription().equalsIgnoreCase(neighbourhoodDescription))
				.filter(item -> item.getOccurrenceReportedYear() == occurrenceReportedYear)
				.sorted(Comparator.comparing(EPSNeighbourhoodCriminalOccurrences::getOccurrenceReportedQuarter)
						.thenComparing(
							Comparator.comparingInt(EPSNeighbourhoodCriminalOccurrences::getOccurrenceReportedMonth))
					)
				.collect(Collectors.toList());
